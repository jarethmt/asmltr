...
test('preface tags asker then two blank lines then body', () => {
  const r = prefaceOnBehalf('313485478869598208', '1½ inch addendum');
  assert.equal(r.ok, true);
  assert.equal(r.text, 'Posting on behalf of <@313485478869598208>\n\n\n1½ inch addendum');
...
test('names are not snowflakes; 666 steak matches a thread title', () => {
  assert.equal(looksLikeSnowflake('1540758871130378341'), true);
