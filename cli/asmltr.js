...
async function cmdSend(rest) {
  ...
  if (!channel || !target || (!text && !file)) {
    throw new Error('usage: asmltr send <channel> <target> "<text>"\n' +
      ...
      '  e.g.  asmltr send discord 123 "shipping now"   ·   asmltr send email a@example.com "the body" --subject "Hello" --cc "boss@example.com" --file /tmp/report.pdf');
  }
...
async function cmdChat() {
  ...
  const chatName = (process.env.ASSISTANT_NAME || 'Assistant').trim() || 'Assistant';
...
function cmdHelp() {
  ...
  asmltr chat            local assistant REPL over the same session (resume UUID)
  asmltr ls              list active sessions
